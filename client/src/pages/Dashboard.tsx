...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const networkRes = await axios.get('/api/network/my-network', { headers: { Authorization: `Bearer ${token}` } });
  ...
  const downloadPacket = () => {
    const token = localStorage.getItem('token');
    if (token) window.open(`/api/packet/emergency-packet?token=${token}`, '_blank');
  };
          ...
          <div className="card">
            ...
            {network.members?.map((m: any) => (
              <div key={m.id} className="member-item">
                <strong>{m.firstName} {m.lastName}</strong> - {m.role}<br />
                <small>{m.email}</small>
              </div>
            ))}
          </div>
